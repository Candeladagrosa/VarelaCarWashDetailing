...
const UsersAdmin = () => {
  ...
  const handleDelete = async (userId) => {
    ...
    if (!window.confirm('⚠️ ¿Estás seguro de que quieres ELIMINAR PERMANENTEMENTE este usuario?\n\nEsta acción NO SE PUEDE DESHACER.\n\n• El usuario será eliminado de la base de datos\n• Se perderán todos sus datos asociados\n• No podrá volver a iniciar sesión')) {
      return;
    }

    try {
      // Eliminar permanentemente el perfil del usuario (hard-delete)
      const { error } = await supabase
        .from('perfiles')
        .delete()
        .eq('id', userId);
      ...
      toast({
        variant: 'destructive',
        title: 'Error',
        description: 'No se pudo eliminar el usuario. Verifica que no tenga datos asociados.',
      });
    }
  };
