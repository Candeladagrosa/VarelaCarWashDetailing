...
const TurnosAdmin = () => {
  ...
  const loadTurnos = async () => {
    ...
    if (clienteIds.length > 0) {
      ...
      if (!perfilesError && perfilesData) {
        ...
        console.warn('No se pudieron cargar perfiles:', perfilesError);
        setPerfiles({});
      }
    }
  ...
  const filteredTurnos = turnos.filter(turno => {
    ...
    const perfil = perfiles[turno.id_cliente];
    const servicio = turno.servicios;
    return (
      (perfil && (
        perfil.nombre?.toLowerCase().includes(search) || 
        perfil.apellido?.toLowerCase().includes(search) ||
        perfil.dni?.toString().includes(search) ||
        perfil.telefono?.toLowerCase().includes(search)
      )) ||
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0">
                <div className="flex-1">
                  ...
                  <p className="text-gray-700 font-semibold">
                    {perfil ? `${perfil.nombre} ${perfil.apellido}` : 'Cliente no encontrado'}
                  </p>
                  {perfil && (
                ...
                <div className="flex items-center space-x-3">
                  <PermissionGuard permission="turnos.cambiar_estado">
                  ...
                  {isEditing ? (
                    <PermissionGuard permission="turnos.editar">
