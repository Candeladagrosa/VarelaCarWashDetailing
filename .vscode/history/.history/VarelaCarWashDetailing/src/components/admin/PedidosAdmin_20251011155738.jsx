...
const PedidosAdmin = () => {
  ...
  const handleProductQuantityChange = (pedidoProductoId, newQuantity) => {
    if (newQuantity < 1) return;
    const updatedProductos = editData.pedido_productos.map(p => 
      p.id_pedido_producto === pedidoProductoId ? { ...p, cantidad: newQuantity } : p
    );
    setEditData({ ...editData, pedido_productos: updatedProductos });
  };

  const handleRemoveProduct = async (pedidoProductoId) => {
    ...
    const { error } = await supabase
      .from('pedido_productos')
      .delete()
      .eq('id_pedido_producto', pedidoProductoId);
    ...
    const updatedProductos = editData.pedido_productos.filter(p => p.id_pedido_producto !== pedidoProductoId);
    setEditData({ ...editData, pedido_productos: updatedProductos });
    toast({ 
            ...
            <motion.div key={pedido.id_pedido} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.05 }} className="glass-effect rounded-2xl p-6">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0 mb-4">
                ...
                <div className="text-right">
                  <p className="text-3xl font-bold gradient-text">${(isEditing ? editData.pedido_productos.reduce((sum, p) => sum + p.precio * p.cantidad, 0) : pedido.total).toLocaleString()}</p>
                  <PermissionGuard permission="pedidos.cambiar_estado">
              ...
              <div className="border-t pt-4">
                ...
                {isEditing ? (
                  <div className="space-y-2">
                    {editData.pedido_productos.map((prod) => (
                      <div key={prod.id_pedido_producto} className="flex items-center justify-between">
                        <span>{prod.productos.nombre}</span>
                        <div className="flex items-center space-x-2">
                          <Button size="icon" variant="ghost" onClick={() => handleProductQuantityChange(prod.id_pedido_producto, prod.cantidad - 1)}><Minus className="w-4 h-4" /></Button>
                          <span>{prod.cantidad}</span>
                          <Button size="icon" variant="ghost" onClick={() => handleProductQuantityChange(prod.id_pedido_producto, prod.cantidad + 1)}><Plus className="w-4 h-4" /></Button>
                          <Button size="icon" variant="ghost" className="text-red-500" onClick={() => handleRemoveProduct(prod.id_pedido_producto)}><Trash2 className="w-4 h-4" /></Button>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <ul className="space-y-1 list-disc list-inside">
                    {pedido.pedido_productos && pedido.pedido_productos.map((prod, idx) => (
                      <li key={idx} className="text-gray-600">{prod.cantidad}x {prod.productos.nombre} - ${prod.precio.toLocaleString()} c/u</li>
                    ))}
                  </ul>
                )}
              </div>
              <div className="flex space-x-2 mt-4">
                {isEditing ? (
                  <PermissionGuard permission="pedidos.editar">
                    <Button onClick={handleSaveEdit} size="sm" className="bg-green-600 hover:bg-green-700" disabled={loading}>
                      <Save className="w-4 h-4 mr-2" />
                      {loading ? 'Guardando...' : 'Guardar'}
                    </Button>
                    <Button onClick={() => { setEditingId(null); setEditData(null); }} size="sm" variant="outline" disabled={loading}>
                  ...
                  <>
                    ...
                    <PermissionGuard permission="pedidos.eliminar">
