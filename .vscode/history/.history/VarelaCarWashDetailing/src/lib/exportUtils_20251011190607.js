 ...
 * @param {Array} turnos - Array de turnos desde Supabase (con joins de usuario y servicio)
 * @returns {Array} - Array formateado para Excel
 */
export const formatTurnosParaExcel = (turnos) => {
  return turnos.map(turno => ({
    'ID': turno.id_turno,
    'Cliente': turno.usuarios?.nombre_completo || 'N/A',
    'Email': turno.usuarios?.email || 'N/A',
    'Servicio': turno.servicios?.nombre || 'N/A',
    'Fecha': new Date(turno.fecha_hora).toLocaleDateString('es-AR'),
    'Hora': new Date(turno.fecha_hora).toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit' }),
    'Estado': turno.estado,
    'Precio': turno.precio_pagado || turno.servicios?.precio || 0,
    'Notas': turno.notas || '',
    'Fecha Creación': new Date(turno.created_at).toLocaleString('es-AR'),
  }));
};
