...
const ServicesAdmin = () => {
  ...
  const handleAdd = () => {
    ...
    const newService = {
      id_servicio: services.length > 0 ? Math.max(...services.map(s => s.id_servicio)) + 1 : 1,
      ...formData,
  ...
  const toggleVisibility = (id) => {
    const updated = services.map(s =>
      s.id_servicio === id ? { ...s, visible: !s.visible } : s
    );
    saveServices(updated);
    toast({ title: 'Visibilidad Cambiada', description: 'Se actualizó la visibilidad del servicio' });
  };

  const filteredServices = services.filter(s => s.estado === 1 && s.nombre.toLowerCase().includes(searchTerm.toLowerCase()));
          ...
          {filteredServices.map((service, index) => (
            <motion.div key={service.id_servicio} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.05 }} className="bg-white/50 rounded-xl p-4 flex justify-between items-center">
              <div className="flex items-center space-x-4">
                <img src={service.imagen || 'https://via.placeholder.com/100'} alt={service.nombre} className="w-16 h-16 rounded-lg object-cover" />
                <div>
