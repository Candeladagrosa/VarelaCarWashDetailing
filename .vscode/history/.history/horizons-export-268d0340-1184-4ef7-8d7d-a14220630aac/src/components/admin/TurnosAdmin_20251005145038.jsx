...
const TurnosAdmin = () => {
  ...
  const saveTurnos = (updatedTurnos) => {
    localStorage.setItem('turnos', JSON.stringify(updatedTurnos));
    setTurnos(updatedTurnos);
  };

  const updateTurnoStatus = (id, newStatus) => {
    const updated = turnos.map(t => t.id_turno === id ? { ...t, estado: newStatus, updated_at: new Date().toISOString() } : t);
    saveTurnos(updated);
    toast({ title: '¡Estado Actualizado! ✅', description: `El turno ha sido ${newStatus.toLowerCase()}` });
  };
  ...
  const handleSaveEdit = () => {
    const updated = turnos.map(t => t.id_turno === editingId ? { ...t, ...editData, updated_at: new Date().toISOString() } : t);
    saveTurnos(updated);
    setEditingId(null);
    toast({ title: '¡Turno Actualizado! ✅', description: 'La fecha y hora del turno han sido modificadas.' });
  };

  const filteredTurnos = turnos.filter(turno => {
    const search = searchTerm.toLowerCase();
    const user = users.find(u => u.id_usuario === turno.id_cliente);
    const service = services.find(s => s.id_servicio === turno.id_servicio);
    return (
      (user && (user.nombre.toLowerCase().includes(search) || user.apellido.toLowerCase().includes(search))) ||
      (service && service.nombre.toLowerCase().includes(search)) ||
            ...
            <motion.div key={turno.id_turno} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.05 }} className="glass-effect rounded-2xl p-6">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0">
                <div className="flex-1">
                  <h3 className="text-xl font-bold text-gray-800 mb-2">{service?.nombre || 'Servicio'}</h3>
                  <p className="text-gray-700 font-semibold">{user ? `${user.nombre} ${user.apellido}` : 'Cliente no encontrado'}</p>
                  {isEditing ? (
                ...
                <div className="flex items-center space-x-3">
                  ...
                  {isEditing ? (
                    <>
                      <Button onClick={handleSaveEdit} size="sm" className="bg-green-600 hover:bg-green-700"><Save className="w-4 h-4" /></Button>
