...
const PedidosAdmin = () => {
  ...
  const savePedidos = (updatedPedidos) => {
    localStorage.setItem('pedidos', JSON.stringify(updatedPedidos));
    setPedidos(updatedPedidos);
  };

  const updatePedidoStatus = (id, type, newStatus) => {
    const updated = pedidos.map(p => p.id_pedido === id ? { ...p, [type]: newStatus } : p);
    savePedidos(updated);
    toast({ title: '¡Estado Actualizado! ✅', description: `El estado de ${type === 'estado_pago' ? 'pago' : 'envío'} ha sido actualizado.` });
  };

  const handleDelete = (id) => {
    if (window.confirm('¿Estás seguro de que quieres eliminar este pedido? Esta acción no se puede deshacer.')) {
      const updated = pedidos.filter(p => p.id_pedido !== id);
      savePedidos(updated);
  ...
  const handleSaveEdit = () => {
    const updatedTotal = editData.productos.reduce((sum, prod) => sum + prod.precio * prod.cantidad, 0);
    const updatedPedido = { ...editData, total: updatedTotal };
    const updated = pedidos.map(p => p.id_pedido === editingId ? updatedPedido : p);
    savePedidos(updated);
    setEditingId(null);
    setEditData(null);
    toast({ title: '¡Pedido Actualizado! ✅', description: 'Los detalles del pedido han sido modificados.' });
  };
